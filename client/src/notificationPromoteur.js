...
class notificationPromoteur extends Component {
    state = {
        web3: null,
        accounts: null,
        chainid: null,
    ...
    componentDidMount = async () => {
        ...
        var nb =  await promoteur.methods.getnbnotification().call()
        this.setState({nbnotification:nb})
       
        for (var i=0; i < nb; i++) {
            const account = await promoteur.methods.getaccountpromoteurnotification(i).call()
            if(account == accounts[0]){
                const stat = await promoteur.methods.getstatusnotfication(i).call()
                if(stat == "Disponible"){
                    const msg = await promoteur.methods.getmessage(i).call()
                    const list =[{
                        message: msg,
                        numnotification : i,
                        status : stat
    ...
    ajouter = async event => {
      ...
      handlesupp = async(numnotification) =>{
        ...
        var result = await promoteur.methods.modifiernotification(numnotification).send({from: accounts[0]})
        this.props.history.push("/notificationPromoteur");
...
export default notificationPromoteur
