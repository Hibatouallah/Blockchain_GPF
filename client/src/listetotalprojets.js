...
class listetotalprojets extends Component {
    ...
    componentDidMount = async () => {
        ...
        for (var i=0; i < nb; i++) {
            ...
            const nb_cli = await fonds.methods.getnb_client(i).call()
            const jard = await fonds.methods.getjardin(i).call()
            const balc = await fonds.methods.getbalcon(i).call()
            const mini_hopi = await fonds.methods.getmini_hopital(i).call()
            const supermar = await fonds.methods.getsupermarche(i).call()
            const ham  = await fonds.methods.gethamam(i).call()
            const mini_mosq = await fonds.methods.getmini_mosque(i).call()
            const pharm = await fonds.methods.getpharmacie(i).call()
            const stat = await fonds.methods.getstatus(i).call()
            ...
            const list =[{
              ...
              jardin : jard,
              balcon: balc, 
              mini_hopital :mini_hopi,
              supermarche : supermar, 
              hamam : ham, 
              mini_mosque :mini_mosq,
              pharmacie : pharm,
              status : stat
            }]
            this.setState({
           ...
           {localStorage.getItem('isfonds') != 'true' &&
            ...
            <Table responsive >
                ...
                {this.state.listeprojet.map((list) =>
                    <tr>
                            <td><center><Image  onClick={() => this.handleaddcontrats(list[0].references)} src={addcontratsicon} /></center></td>
                            <td><Image onClick={this.handlepomoteur} src={updateicon} roundedCircle /></td>
                            <td>{list[0].references}</td>
                            <td><Card.Img variant="top" src={`https://ipfs.infura.io/ipfs/${list[0].image}`}/></td>
                            <td><Card.Img variant="top" src={`https://ipfs.infura.io/ipfs/${list[0].imagesec1}`} /></td>
                            <td><Card.Img variant="top" src={`https://ipfs.infura.io/ipfs/${list[0].imagesec2}`} /></td>
                            <td><Card.Img variant="top" src={`https://ipfs.infura.io/ipfs/${list[0].imagesec3}`} /></td>
                            ...
                            <td><a href={`https://ipfs.infura.io/ipfs/${list[0].mesures_securites_hygiene}`}>mesures_securites_hygiene.pdf</a></td>
                            <td>{list[0].reception_provisoire_travaux}</td>
                            <td>{list[0].reception_definitive}</td>
                            <td><a href={`https://ipfs.infura.io/ipfs/${list[0].cahier_prestations_techniques}`}>cahier_prestations_techniques.pdf</a></td>
                            <td><a href={`https://ipfs.infura.io/ipfs/${list[0].normes_mise_en_oeuvre}`}>normes_mise_en_oeuvre.pdf</a></td>
                            <td>{list[0].localisation}</td>
                            <td>{list[0].Descriptif}</td>
                            <td>{list[0].nb_client}</td>
                            ...
                            <td>{list[0].jardin}</td>
                            <td>{list[0].balcon}</td> 
                            <td>{list[0].mini_hopital}</td>
                            <td>{list[0].supermarche}</td> 
                            <td>{list[0].hamam}</td>
                            <td>{list[0].mini_mosque}</td> 
                            <td>{list[0].pharmacie}</td>
                            <td>{list[0].status}</td>
                    </tr>
...
export default listetotalprojets
