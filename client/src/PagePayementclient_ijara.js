...
class PagePayementclient_ijara extends Component {
        ...
        for (var i=0; i < nb; i++) {
                ...
                if(cin == localStorage.getItem("cinclientpayer")){
                    ...
                    if(ref == localStorage.getItem("refprojetpayer")){
                        ...
                        const montant = await engagementclient.methods.getmontantpartranche(i).call()
        ...
        for(var k = 0;k<nbl;k++){
            ...
            if(cinpay == localStorage.getItem("cinclientpayer")){
                ...
                if(refpay == this.state.reference){
                    ...
                    var result = await engagementclient.methods.modifier_moisactuelpayement_ijara(k,moisactuel).send({from: accounts[0]})
    ...
    if(mois == 2){
        for(var k = 0;k<nbl;k++){
            ...
            if(cinpay == localStorage.getItem("cinclientpayer")){
                const refpay= await engagementclient.methods.getreferenceprojetijara(k).call()
                if(refpay == this.state.reference){
                    const duree= await engagementclient.methods.getduree_payement_ijara(k).call()
                    ...
                    if(anneeac == duree){
                        ...
                        var result1 = await fonds.methods.ajouternotification(_message).send({from: accounts[0]})  
                       // Stocker historique de payement
                       var res = await engagementclient.methods.ajouterpayementhistorique(localStorage.getItem("cinclientpayer"),this.state.reference,_message).send({from: accounts[0]})
                      // Fin Payement
                      for(var l = 0;l<nbc;l++){
                        ...
                        if(ci == localStorage.getItem("cinclient")){
                            ...
                            if(ref == this.state.reference){
                               var res = await engagementclient.methods.modifierpayement_client(l,'Paiement effectuée').send({from: accounts[0]})
                                }
                        }
                      }
                      alert('Payement est effectué avec succés')
                    ...
                    if(anneeac < duree){
                        ...
                        var result = await engagementclient.methods.modifier_anneeactuelpayement_ijara(k,anneeactuel).send({from: accounts[0]})
                        var result2 = await engagementclient.methods.modifier_moisactuelpayement_ijara(k,'1').send({from: accounts[0]})
                        var _message = "La tranche du mois "+ mois +"de l'année "+anneeac+"a été payer par le client "+localStorage.getItem("cinclientpayer")+",le projet avec la reference : "+this.state.reference
                        var result1 = await fonds.methods.ajouternotification(_message).send({from: accounts[0]})  
        ...
        for(var k = 0;k<nbl;k++){
            ...
            if(cinpay == localStorage.getItem("cinclientpayer")){
                ...
                if(refpay == this.state.reference){
                    ...
                    var result = await engagementclient.methods.modifier_semestreactuelpayement_ijara(k,semstreactuel).send({from: accounts[0]})
            ...
            if(cinpay == localStorage.getItem("cinclientpayer")){
                ...
                if(refpay == this.state.reference){
                    ...
                    if(anneeac < duree){
                        ...
                        var anneeactuel = (parseInt(anneeac)+1).toString()
                        var result = await engagementclient.methods.modifier_anneeactuelpayement_ijara(k,anneeactuel).send({from: accounts[0]})
                        var result2 = await engagementclient.methods.modifier_semestreactuelpayement_ijara(k,'1').send({from: accounts[0]})
                        var _message = "La tranche du semestre "+ semestre +"de l'année "+anneeac+"a été payer par le client "+localStorage.getItem("cinclientpayer")+",le projet avec la reference : "+this.state.reference
                        var result1 = await fonds.methods.ajouternotification(_message).send({from: accounts[0]})  
        ...
        for(var k = 0;k<nbl;k++){
            ...
            if(cinpay == localStorage.getItem("cinclientpayer")){
                ...
                if(refpay == this.state.reference){
                    ...
                    var result = await engagementclient.methods.modifier_trimestreactuelpayement_ijara(k,trimestreactuel).send({from: accounts[0]})
            ...
            if(cinpay == localStorage.getItem("cinclientpayer")){
                ...
                if(refpay == this.state.reference){
                    ...
                    if(anneeac < duree){
                        ...
                        var anneeactuel = (parseInt(anneeac)+1).toString()
                        var result = await engagementclient.methods.modifier_anneeactuelpayement_ijara(k,anneeactuel).send({from: accounts[0]})
                        var result2 = await engagementclient.methods.modifier_trimestreactuelpayement_ijara(k,'1').send({from: accounts[0]})
...
export default PagePayementclient_ijara
