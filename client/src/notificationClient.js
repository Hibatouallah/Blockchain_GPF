...
class notificationClient extends Component {
    state = {
        web3: null,
        accounts: null,
        chainid: null,
    ...
    componentDidMount = async () => {
        ...
        var nb =  await client.methods.getnbnotification().call()
        this.setState({nbnotification:nb})
       
        for (var i=0; i < nb; i++) {
            const account = await client.methods.getaccountclientnotification(i).call()
            if(account == accounts[0]){
                const stat = await client.methods.getstatusnotfication(i).call()
                if(stat == "Disponible"){
                    const msg = await client.methods.getmessage(i).call()
                    const list =[{
                        message: msg,
                        numnotification : i,
                        status : stat
    ...
    ajouter = async event => {
      ...
      handlesupp = async(numnotification) =>{
        ...
        this.props.history.push("/notificationClient");
