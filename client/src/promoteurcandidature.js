...
class promoteurcandidature extends Component {
    ...
    componentDidMount = async () => {
        ...
        var nbnoti = await promoteurs.methods.getnbnotification().call()
      
        var nb =  await promoteurs.methods.listecandidature().call()
        this.setState({nbcandidature:nb})
        for (var i=0; i < nb; i++) {
            var account = await promoteurs.methods.getaccountpromoteur(i).call()
            if(account == accounts[0]){
                ...
                const conf = await promoteurs.methods.getconfirmation(i).call()
                const list =[{
                    reference: ref, 
    ...
    handleChange = async(ref) =>{
        localStorage.setItem("reference_proj_eng_pro",ref)
        this.props.history.push("/engagementdefinitif_promo");
    }
    render() {
            ...
            <Table responsive >
                ...
                {this.state.listecandidature.map((list) =>
                    <tr>
                            ...
                            {list[0].confirmation === 'oui' &&
                                <td><Button variant="dark" onClick={() => this.handleChange(list[0].reference)}>Completer la procedure</Button></td>
                            }
                             {list[0].confirmation != 'oui' &&
                                <td>Pas encore</td>
                            }
...
export default promoteurcandidature
